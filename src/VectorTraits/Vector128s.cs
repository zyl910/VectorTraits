...
namespace Zyl.VectorTraits {
    ...
    /// <summary>
    /// Methods of <see cref="Vector128{T}"/>. It supports ExType, and the element type can also be (它支持扩展类型, 元素类型还可以为): <see cref="ExInt128"/>, <see cref="ExUInt128"/>, <see cref="Int128"/>, <see cref="UInt128"/>.
    /// </summary>
    public static partial class Vector128s {
        ...
        static Vector128s() {
            unchecked {
                ...
                MaskBitPosArray16B = new Vector128<byte>[16 * 8];
                ...
                MaskBitsArray16B = new Vector128<byte>[16 * 8 + 1];
                ...
                MaskBitsArray16B[0] = Vector128<byte>.Zero;
                bitpos = 1;
                bits = 1;
                for (i = 0; i < MaskBitPosArray16B.Length; ++i) {
                    if (i < MaskBitPosArray1B.Length) {
                        MaskBitPosArray1B[i] = Vector128.Create(Scalars.GetBy128Bits<byte>(bitpos));
                        MaskBitsArray1B[1 + i] = Vector128.Create(Scalars.GetBy128Bits<byte>(bits));
                    }
                    if (i < MaskBitPosArray2B.Length) {
                        MaskBitPosArray2B[i] = Vector128.AsByte(Vector128.Create(Scalars.GetBy128Bits<ushort>(bitpos)));
                        MaskBitsArray2B[1 + i] = Vector128.AsByte(Vector128.Create(Scalars.GetBy128Bits<ushort>(bits)));
                    }
                    if (i < MaskBitPosArray4B.Length) {
                        MaskBitPosArray4B[i] = Vector128.AsByte(Vector128.Create(Scalars.GetBy128Bits<uint>(bitpos)));
                        MaskBitsArray4B[1 + i] = Vector128.AsByte(Vector128.Create(Scalars.GetBy128Bits<uint>(bits)));
                    }
                    if (i < MaskBitPosArray8B.Length) {
                        MaskBitPosArray8B[i] = Vector128.AsByte(Vector128.Create(Scalars.GetBy128Bits<ulong>(bitpos)));
                        MaskBitsArray8B[1 + i] = Vector128.AsByte(Vector128.Create(Scalars.GetBy128Bits<ulong>(bits)));
                    }
        ...
        public static int Count<T>() where T : struct {
            return Vector128<byte>.Count / Unsafe.SizeOf<T>();
        }
        ...
        /// <returns>A new <see cref="Vector64{T}" /> initialized from <paramref name="lower" /> and <paramref name="upper" /> (从 <paramref name="lower" /> 和 <paramref name="upper" /> 初始化的新 <see cref="Vector64{T}" />).</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Vector128<T> Create<T>(Vector64<T> lower, Vector64<T> upper) where T : struct {
            return Vector128.Create(lower.ExAsUInt64(), upper.ExAsUInt64()).ExAs<ulong, T>();
        }
        ...
        /// <seealso cref="Vector128.Create{T}(T)"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Vector128<T> Create<T>(T value) where T : struct {
            if (typeof(T) == typeof(float)) {
                ...
                return Vector128s.Create(Unsafe.As<T, ExInt128>(ref value)).ExAs<ExInt128, T>();
            } else if (typeof(T) == typeof(ExUInt128)) {
                return Vector128s.Create(Unsafe.As<T, ExUInt128>(ref value)).ExAs<ExUInt128, T>();
#if BCL_TYPE_INT128
            } else if (typeof(T) == typeof(Int128)) {
                return Vector128s.Create(Unsafe.As<T, Int128>(ref value)).ExAs<Int128, T>();
            } else if (typeof(T) == typeof(UInt128)) {
                return Vector128s.Create(Unsafe.As<T, UInt128>(ref value)).ExAs<UInt128, T>();
        ...
        public static Vector128<T> Create<T>(T[] values) where T : struct {
            // return Vector128.Create(values); // Need NET7_0_OR_GREATER. But not support ExType.
        ...
        /// <seealso cref="Vector128.Create{T}(T[], int)"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <seealso cref="Vector128.Create{T}(ReadOnlySpan{T})"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        public static Vector128<T> CreateRotate<T>(ReadOnlySpan<T> values, int index, int length) where T : struct {
            ...
            UnsafeUtil.SkipInit(out Vector128<T> temp);
            ref T p = ref Unsafe.As<Vector128<T>, T>(ref temp);
        ...
        /// <summary>
        /// Rotate creates a new <see cref="Vector128{T}"/> from a given span (从指定跨度旋转创建一个 <see cref="Vector128{T}"/>).
        /// </summary>
        /// <typeparam name="T">The vector element type (向量中的元素的类型).</typeparam>
        /// <param name="values">The span from which the vector is created (用于创建向量的跨度).</param>
        /// <returns>A new <see cref="Vector128{T}"/> with its elements set to the first Count elements from <paramref name="values"/> (一个新<see cref="Vector128{T}"/>，其元素设置为来自<paramref name="values"/>首批满足长度的元素).</returns>
        ...
        /// <returns>A new <see cref="Vector128{T}"/> with all elements initialized to value (一个新的 <see cref="Vector128{T}"/>，其中所有元素已初始化为 <paramref name="src"/> ).</returns>
        ...
        /// <summary>
        /// Creates a <see cref="Vector128{T}"/> whose components are of a specified 128-bits integer (创建一个 <see cref="Vector128{T}"/>，其元素为指定的128位整数).
        /// </summary>
        /// <typeparam name="T">The vector element type (向量中的元素的类型).</typeparam>
        /// <param name="src">Source value (源值).</param>
        /// <returns>A new <see cref="Vector128{T}"/> with all elements initialized to value (一个新的 <see cref="Vector128{T}"/>，其中所有元素已初始化为 <paramref name="src"/> ).</returns>
        public static Vector128<T> CreateBy128Bits<T>(ExInt128 src) where T : struct {
        ...
        /// <seealso cref="Vector128{T}.IsSupported"/>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
