...
namespace Zyl.VectorTraits {
    ...
    /// </summary>
    public static partial class Vector128s {
#if NETCOREAPP3_0_OR_GREATER
            ...
            unchecked {
                ...
                for (i = 0; i < MaskBitPosArray8B.Length; ++i) {
                    ...
                    if (i < MaskBitPosArray2B.Length) {
                        MaskBitPosArray2B[i] = Vector128.AsByte(Vector128.Create(Scalars.GetByBits<ushort>(bitpos)));
                        MaskBitsArray2B[1 + i] = Vector128.AsByte(Vector128.Create(Scalars.GetByBits<ushort>(bits)));
                    }
                    if (i < MaskBitPosArray4B.Length) {
                        MaskBitPosArray4B[i] = Vector128.AsByte(Vector128.Create(Scalars.GetByBits<uint>(bitpos)));
                        MaskBitsArray4B[1 + i] = Vector128.AsByte(Vector128.Create(Scalars.GetByBits<uint>(bits)));
                    }
                    if (i < MaskBitPosArray8B.Length) {
                        MaskBitPosArray8B[i] = Vector128.AsByte(Vector128.Create(Scalars.GetByBits<ulong>(bitpos)));
                        MaskBitsArray8B[1 + i] = Vector128.AsByte(Vector128.Create(Scalars.GetByBits<ulong>(bits)));
                    }
                    // next.
                    bitpos <<= 1;
                    bits = bits << 1 | 1;
        ...
        public static Vector128<T> CreatePadding<T>(ReadOnlySpan<T> values, int index, int length) where T : struct {
            ...
            ref T p = ref Unsafe.As<Vector128<T>, T>(ref temp);
            int m = Math.Min(Vector128<T>.Count, length);
            for (int i = 0; i < m; ++i) {
                p = values[idx];
                ++idx;
        ...
        public static Vector128<T> CreateRotate<T>(ReadOnlySpan<T> values, int index, int length) where T : struct {
            ...
            UnsafeEx.SkipInit(out Vector128<T> temp);
            ref T p = ref Unsafe.As<Vector128<T>, T>(ref temp);
        ...
        /// <param name="src">Source value (源值).</param>
        /// <returns>A new <see cref="Vector128{T}"/> with all elements initialized to value (一个新的 <see cref="Vector128{T}"/>，其中所有元素已初始化为 <paramref name="value"/> ).</returns>
        public static Vector128<T> CreateByBits<T>(long src) where T : struct {
        ...
        internal static Vector128<T> BaseOnesComplement<T>(Vector128<T> src) where T : struct {
            ...
            return ~src;
#else
            ref ulong p = ref Unsafe.As<Vector128<T>, ulong>(ref src);
            p = ~p;
