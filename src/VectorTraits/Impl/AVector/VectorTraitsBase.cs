...
//#endif // NET5_0_OR_GREATER
            ...
            public static Vector<double> YTruncate_ClearBit(Vector<double> value) {
                //constants.
                Vector<double> signMask = new Vector<double>(ScalarConstants.DoubleVal_SignMask);
                // operations
            ...
            /// <inheritdoc cref="IVectorTraits.YTruncate(Vector{double})"/>
            [Obsolete("It is a highly readable version with weaker performance. Please use YTruncate_ClearBit instead (这是一个高度可读的版本, 但性能较弱. 请用YTruncate_ClearBit代替). ")]
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public static Vector<double> YTruncate_ClearBit_Readability(Vector<double> value) {
                //constants.
                Vector<double> signMask = new Vector<double>(ScalarConstants.DoubleVal_SignMask);
                Vector<double> exponentMask = new Vector<double>(ScalarConstants.DoubleVal_ExponentMask);
                Vector<long> expMinuend = new Vector<long>(((long)ScalarConstants.Double_ExponentBias * 2 + ScalarConstants.Double_ExponentShift) << ScalarConstants.Double_ExponentShift); // Item is `(long)(1023*2 + 52)<<52`. Binary is `0x8320000000000000`.
