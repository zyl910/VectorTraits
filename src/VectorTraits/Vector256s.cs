            ...
            unchecked {
                ...
                for (i = 0; i < MaskBitPosArray8B.Length; ++i) {
                    ...
                    if (i < MaskBitPosArray2B.Length) {
                        MaskBitPosArray2B[i] = Vector256.AsByte(Vector256.Create(Scalars.GetByBits<ushort>(bitpos)));
                        MaskBitsArray2B[1 + i] = Vector256.AsByte(Vector256.Create(Scalars.GetByBits<ushort>(bits)));
                    }
                    if (i < MaskBitPosArray4B.Length) {
                        MaskBitPosArray4B[i] = Vector256.AsByte(Vector256.Create(Scalars.GetByBits<uint>(bitpos)));
                        MaskBitsArray4B[1 + i] = Vector256.AsByte(Vector256.Create(Scalars.GetByBits<uint>(bits)));
                    }
                    if (i < MaskBitPosArray8B.Length) {
                        MaskBitPosArray8B[i] = Vector256.AsByte(Vector256.Create(Scalars.GetByBits<ulong>(bitpos)));
                        MaskBitsArray8B[1 + i] = Vector256.AsByte(Vector256.Create(Scalars.GetByBits<ulong>(bits)));
                    }
                    // next.
                    bitpos <<= 1;
                    bits = bits << 1 | 1;
