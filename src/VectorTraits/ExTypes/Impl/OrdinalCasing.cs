...
#else
            new ushort[0];
#endif // NETSTANDARD1_3_OR_GREATER || NETCOREAPP1_0_OR_GREATER || NET46_OR_GREATER
...
*/
        ...
        /*
        private static ushort[] InitOrdinalCasingPage(int pageNumber) {
            Debug.Assert(pageNumber >= 0 && pageNumber < 256);

            ushort[] casingTable = new ushort[256];
            if (true) {
                ref ushort table = ref casingTable[0];
                ref char pTable = ref Unsafe.As<ushort, char>(ref table);
