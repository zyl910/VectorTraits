...
namespace Zyl.VectorTraits {
    ...
    public abstract class Vector256s<T> : AbstractVectors<T> where T : struct {
        ...
        static Vector256s() {
            ...
            // -- Specified value --
            IndexMask = Vector256s.CreateByBits<T>(Vector256s.Count<T>() - 1);
            IndexMaskEach128 = Vector256s.CreateByBits<T>((16 / Scalars<T>.ByteSize) - 1);
            IndexX2Mask = Vector256s.CreateByBits<T>(Vector256s.Count<T>() * 2 - 1);
            IndexX4Mask = Vector256s.CreateByBits<T>(Vector256s.Count<T>() * 4 - 1);
            Serial = Vector256s.CreateByDoubleLoop<T>(0, 1);
            SerialDesc = Vector256s.CreateByDoubleLoop<T>(Vector256s.Count<T>() - 1, -1);
        ...
        public static Vector256<T> GetMaskBitPos(int index) {
            return Unsafe.As<Vector256<byte>, Vector256<T>>(ref MaskBitPosArray[index]);
        }

        /// <summary>
        /// Get bit pos mask span (获取位偏移掩码的跨度). Tip: You can use <see cref="Unsafe.As{TFrom, TTo}(ref TFrom)"/> convert its item to <see cref="Vector256{T}"/> type (提示: 可以用 <see cref="Unsafe.As{TFrom, TTo}(ref TFrom)"/> 将其中条目转为 <see cref="Vector256{T}"/> 类型).
        /// </summary>
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Vector256<T> GetMaskBits(int index) {
            return Unsafe.As<Vector256<byte>, Vector256<T>>(ref MaskBitsArray[index]);
        }

        /// <summary>
        /// Get bits mask span (获取位集掩码的跨度). Tip: You can use <see cref="Unsafe.As{TFrom, TTo}(ref TFrom)"/> convert its item to <see cref="Vector256{T}"/> type (提示: 可以用 <see cref="Unsafe.As{TFrom, TTo}(ref TFrom)"/> 将其中条目转为 <see cref="Vector256{T}"/> 类型).
        /// </summary>
        ...
        /// <seealso cref="Vector256{T}.Zero"/>
        public static Vector256<T> Zero { [MethodImpl(MethodImplOptions.AggressiveInlining)] get { return Vector256<byte>.Zero.ExAs<byte, T>(); } }
        /// <summary>Value 0 (0的值).</summary>
        /// <seealso cref="Vector256{T}.Zero"/>
        public static Vector256<T> V0 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get { return Vector256<byte>.Zero.ExAs<byte, T>(); } }
        /// <summary>All bit is 1 (所有位都是1的值).</summary>
        /// <seealso cref="Vector256{T}.AllBitsSet"/>
...
#else
                return Vector256.Create((int)-1).As<int, T>();
#endif // NET5_0_OR_GREATER
            }
        }

        /// <summary>1 bits mask (1位掩码).</summary>
        public static Vector256<T> MaskBits1 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get { return GetMaskBits(1); } }
        /// <summary>2 bits mask (2位掩码).</summary>
        public static Vector256<T> MaskBits2 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get { return GetMaskBits(2); } }
        /// <summary>4 bits mask (4位掩码).</summary>
        public static Vector256<T> MaskBits4 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get { return GetMaskBits(4); } }
        /// <summary>8 bits mask (8位掩码).</summary>
        public static Vector256<T> MaskBits8 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get { return GetMaskBits(8); } }
        /// <summary>16 bits mask (16位掩码).</summary>
        public static Vector256<T> MaskBits16 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get { return GetMaskBits(Math.Min(ElementBitSize, 16)); } }
        /// <summary>32 bits mask (32位掩码).</summary>
        public static Vector256<T> MaskBits32 { [MethodImpl(MethodImplOptions.AggressiveInlining)] get { return GetMaskBits(Math.Min(ElementBitSize, 32)); } }
