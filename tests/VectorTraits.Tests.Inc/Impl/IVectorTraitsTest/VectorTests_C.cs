...
namespace Zyl.VectorTraits.Tests.Impl.IVectorTraitsTest {
    ...
    public class VectorTests_C {
        ...
        public void ConvertToInt64_Range52Test<T>(T src) where T : struct {
            ...
            var funcList = Vectors.GetSupportedMethodList<Func<Vector<T>, Vector<long>>>("ConvertToInt64_Range52", "ConvertToInt64_Range52_Impl", "ConvertToInt64_Range52RoundToEven");
            foreach (var func in funcList) {
                Console.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            Console.WriteLine();
            ...
            foreach (Vector<T> value in samples) {
                ...
                foreach (IVectorTraits instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector<long> dst = instance.ConvertToInt64_Range52((dynamic)value);
                    if (allowLog) {
                        Console.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void ConvertToUInt64_Range52Test<T>(T src) where T : struct {
            ...
            foreach (Vector<T> value in samples) {
                Console.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector<ulong> expected = Vectors.ConvertToUInt64_Range52((dynamic)value);
                Console.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IVectorTraits instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector<ulong> dst = instance.ConvertToUInt64_Range52((dynamic)value);
                    if (allowLog) {
                        Console.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
            ...
            if (rangeStarts.Length > 0) {
                ...
                foreach (double start in rangeStarts) {
                    for (int i = 0; i < rangeItemCountVector; ++i) {
                        double startNumber = start + Vector<T>.Count * i * stepDelta;
                        Vector<T> value = Vectors.CreateByDoubleLoop<T>(startNumber, stepDelta);
                        Vector<ulong> expected = Vectors.BaseInstance.ConvertToUInt64((dynamic)value);
                        bool usedWrite = false;
                        // funcList.
                        foreach (var func in funcList) {
                            string funcName = ReflectionUtil.GetShortNameWithType(func.Method);
