...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public void YGroup2TransposeTest<T>(T src) where T : struct {
            ...
            var funcList = Vectors.GetSupportedMethodList<FuncIn2Out1<Vector<T>>>("YGroup2Transpose_AlignRight", "YGroup2Transpose_Basic", "YGroup2Transpose_Move", "YGroup2Transpose_Shift", "YGroup2Transpose_ShiftByMul", "YGroup2Transpose_Shuffle", "YGroup2Transpose_ShuffleX", "YGroup2Transpose_ShuffleXImm", "YGroup2Transpose_Unpack");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            if (!MyTestUtil.AllowDelegateOut) funcList.Clear();
            ...
            for (int i = 0; i < samples.Length; i++) {
                ...
                for (int j = 0; j < samples.Length; j++) {
                    ...
                    Vector<T> dst0, dst1;
#pragma warning disable CS0618 // Type or member is obsolete
                    (Vector<T> expected0, Vector<T> expected1) = Vectors.YGroup2Transpose(data0, data1);
                    if (allowLog && 0 == i && 1 == j) {
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public void YGroup2TransposeEvenTest<T>(T src) where T : struct {
            ...
            var funcList = Vectors.GetSupportedMethodList<Func<Vector<T>, Vector<T>, Vector<T>>>("YGroup2TransposeEven_Basic", "YGroup2TransposeEven_Shift", "YGroup2TransposeEven_Shuffle", "YGroup2TransposeEven_ShuffleX", "YGroup2TransposeEven_ShuffleXImm");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            ...
            for (int i = 0; i < samples.Length; i++) {
                ...
                for (int j = 0; j < samples.Length; j++) {
                    ...
                    Vector<T> dst;
#pragma warning disable CS0618 // Type or member is obsolete
                    Vector<T> expected = Vectors.YGroup2TransposeEven((dynamic)data0, (dynamic)data1);
                    if (allowLog && 0 == i && 1 == j) {
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public void YGroup2TransposeOddTest<T>(T src) where T : struct {
            ...
            var funcList = Vectors.GetSupportedMethodList<Func<Vector<T>, Vector<T>, Vector<T>>>("YGroup2TransposeOdd_Basic", "YGroup2TransposeOdd_Shift", "YGroup2TransposeOdd_Shuffle", "YGroup2TransposeOdd_ShuffleX", "YGroup2TransposeOdd_ShuffleXImm");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            if (!MyTestUtil.AllowDelegateMultiParam) funcList.Clear();
            ...
            for (int i = 0; i < samples.Length; i++) {
                ...
                for (int j = 0; j < samples.Length; j++) {
                    ...
                    Vector<T> dst;
#pragma warning disable CS0618 // Type or member is obsolete
                    Vector<T> expected = Vectors.YGroup2TransposeOdd((dynamic)data0, (dynamic)data1);
                    if (allowLog && 0 == i && 1 == j) {
                        writer.WriteLine(VectorTextUtil.Format("f({0}, {1}): {2}", data0, data1, expected));
                    }
                    foreach (IVectorTraits instance in instances) {
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public void YGroup2UnzipTest<T>(T src) where T : struct {
            ...
            var funcList = Vectors.GetSupportedMethodList<FuncIn2Out1<Vector<T>>>("YGroup2Unzip_Basic", "YGroup2Unzip_Narrow", "YGroup2Unzip_NarrowMul", "YGroup2Unzip_Permute", "YGroup2Unzip_Shuffle", "YGroup2Unzip_ShuffleX", "YGroup2Unzip_ShuffleXImm", "YGroup2Unzip_Unpack");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            if (!MyTestUtil.AllowDelegateOut) funcList.Clear();
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public void YGroup2ZipTest<T>(T src) where T : struct {
            ...
            var funcList = Vectors.GetSupportedMethodList<FuncIn2Out1<Vector<T>>>("YGroup2Zip_Basic", "YGroup2Zip_Permute", "YGroup2Zip_Shuffle", "YGroup2Zip_ShuffleX", "YGroup2Zip_Unpack", "YGroup2Zip_Widen", "YGroup2Zip_WidenMul");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            if (!MyTestUtil.AllowDelegateOut) funcList.Clear();
