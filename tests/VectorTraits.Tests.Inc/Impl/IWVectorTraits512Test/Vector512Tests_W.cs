...
#pragma warning disable CS0618 // Type or member is obsolete
            ...
            foreach (Vector512<T> source in samples) {
                ...
                Vector512Generic.Widen(source, out expected0, out expected1);
#else
                ...
                Vector512s.Widen(source, out expected0, out expected1);
...
#pragma warning disable CS0618 // Type or member is obsolete
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("WidenLower_Basic");
            ...
            foreach (Vector512<T> source in samples) {
                ...
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst0 = instance.WidenLower((dynamic)source);
                    if (allowLogItem) {
                ...
                foreach (var func in funcList) {
                    ...
                    Vector512<TOut> dst0 = func((dynamic)source);
                    if (allowLogItem) {
...
#pragma warning disable CS0618 // Type or member is obsolete
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("WidenUpper_Basic");
            foreach (var func in funcList) {
                Console.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            Console.WriteLine();
            ...
            foreach (Vector512<T> source in samples) {
                ...
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst0 = instance.WidenUpper((dynamic)source);
                    if (allowLogItem) {
                        Console.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst0));
                    } else {
                        ClassicAssert.AreEqual(expected, dst0, $"{instance.GetType().Name}, source={source}");
