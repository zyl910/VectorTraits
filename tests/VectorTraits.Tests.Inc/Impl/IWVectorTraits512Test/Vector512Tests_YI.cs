...
namespace Zyl.VectorTraits.Tests.Impl.IWVectorTraits512Test {
    [TestFixture()]
    public class Vector512Tests_YI {
        ...
        public void YIsAllTrueTest<T>(T src) where T : struct {
            ...
            foreach (Vector512<T> left in samples) {
                foreach (Vector512<T> right in samples) {
                    ...
                    Vector512<T> expectedSrc = Vector512.Equals(left, right);
#else
                    Vector512<T> expectedSrc = Vector512s.Equals((dynamic)left, (dynamic)right);
#endif // NET7_0_OR_GREATER
                    bool expected = Vector512s.YIsAllTrue(expectedSrc);
                    foreach (IWVectorTraits512 instance in instances) {
        ...
        public void YIsAnyTrueTest<T>(T src) where T : struct {
            ...
            foreach (Vector512<T> left in samples) {
                foreach (Vector512<T> right in samples) {
                    ...
                    Vector512<T> expectedSrc = Vector512s.Equals((dynamic)left, (dynamic)right);
#endif // NET7_0_OR_GREATER
                    bool expected = Vector512s.YIsAnyTrue(expectedSrc);
                    foreach (IWVectorTraits512 instance in instances) {
                        if (!instance.GetIsSupported(true)) continue;
        ...
        public void YIsEvenIntegerTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsEvenInteger_Basic", "YIsEvenInteger_Bit");
            ...
            foreach (Vector512<T> value in samples) {
                ...
                Vector512<TOut> expected = Vector512s.YIsEvenInteger((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsEvenInteger((dynamic)value);
        ...
        public void YIsFiniteTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsFinite_Basic", "YIsFinite_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsFinite((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsFinite((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsInfinityTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsInfinity_Basic", "YIsInfinity_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsInfinity((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsInfinity((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsInfinityOrNaNTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsInfinityOrNaN_Basic", "YIsInfinityOrNaN_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsInfinityOrNaN((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsInfinityOrNaN((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsIntegerTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsInteger_Basic", "YIsInteger_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsInteger((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsInteger((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsNaNTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsNaN_Basic", "YIsNaN_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsNaN((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsNaN((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsNegativeTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsNegative_Basic", "YIsNegative_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsNegative((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsNegative((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsNegativeInfinityTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsNegativeInfinity_Basic", "YIsNegativeInfinity_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsNegativeInfinity((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsNegativeInfinity((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsNegativeZeroTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsNegativeZero_Basic", "YIsNegativeZero_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsNegativeZero((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsNegativeZero((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsNormalTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsNormal_Basic", "YIsNormal_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsNormal((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsNormal((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsNotEqualsTest<T>(T src) where T : struct {
            ...
            foreach (Vector512<T> left in samples) {
                foreach (Vector512<T> right in samples) {
                    Vector512<T> expected = Vector512s.YIsNotEquals((dynamic)left, (dynamic)right);
                    foreach (IWVectorTraits512 instance in instances) {
                        if (!instance.GetIsSupported(true)) continue;
                        Vector512<T> dst = instance.YIsNotEquals((dynamic)left, (dynamic)right);
        ...
        public void YIsNotNaNTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsNotNaN_Basic", "YIsNotNaN_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsNotNaN((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsNotNaN((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsOddIntegerTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct
        {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsOddInteger_Basic", "YIsOddInteger_Bit");
            foreach (var func in funcList)
            {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            ...
            {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsOddInteger((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances)
                {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsOddInteger((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst)))
                    {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else
        ...
        public void YIsPositiveTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsPositive_Basic", "YIsPositive_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsPositive((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsPositive((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsPositiveInfinityTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsPositiveInfinity_Basic", "YIsPositiveInfinity_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsPositiveInfinity((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsPositiveInfinity((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsSubnormalTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsSubnormal_Basic", "YIsSubnormal_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsSubnormal((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsSubnormal((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsZeroTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsZero_Basic", "YIsZero_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsZero((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsZero((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsZeroOrSubnormalTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<Func<Vector512<T>, Vector512<TOut>>>("YIsZeroOrSubnormal_Basic", "YIsZeroOrSubnormal_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector512<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector512<TOut> expected = Vector512s.YIsZeroOrSubnormal((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits512 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector512<TOut> dst = instance.YIsZeroOrSubnormal((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        Assert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
