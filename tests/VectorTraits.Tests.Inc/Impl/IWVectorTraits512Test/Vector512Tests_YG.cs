...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public void YGroup2TransposeTest<T>(T src) where T : struct {
            ...
            var funcList = Vector512s.GetSupportedMethodList<FuncIn2Out1<Vector512<T>>>("YGroup2Transpose_Basic", "YGroup2Transpose_Shift", "YGroup2Transpose_Shuffle", "YGroup2Transpose_ShuffleX", "YGroup2Transpose_ShuffleXImm");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            if (!MyTestUtil.AllowDelegateOut) funcList.Clear();
