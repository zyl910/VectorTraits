...
namespace Zyl.VectorTraits.Tests.Impl.IWVectorTraits256Test {
    ...
    public class Vector256Tests_YI {
        ...
        public void YIsNotEqualsTest<T>(T src) where T : struct {
            ...
            foreach (Vector256<T> left in samples) {
                foreach (Vector256<T> right in samples) {
                    Vector256<T> expected = Vector256s.YIsNotEquals((dynamic)left, (dynamic)right);
                    foreach (IWVectorTraits256 instance in instances) {
                        if (!instance.GetIsSupported(true)) continue;
                        Vector256<T> dst = instance.YIsNotEquals((dynamic)left, (dynamic)right);
        ...
        public void YIsOddIntegerTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct
        {
            ...
            var funcList = Vector256s.GetSupportedMethodList<Func<Vector256<T>, Vector256<TOut>>>("YIsOddInteger_Basic", "YIsOddInteger_Bit");
            foreach (var func in funcList)
            {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            ...
            {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector256<TOut> expected = Vector256s.YIsOddInteger((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits256 instance in instances)
                {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector256<TOut> dst = instance.YIsOddInteger((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst)))
                    {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else
        ...
        public void YIsPositiveTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector256s.GetSupportedMethodList<Func<Vector256<T>, Vector256<TOut>>>("YIsPositive_Basic", "YIsPositive_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector256<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector256<TOut> expected = Vector256s.YIsPositive((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits256 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector256<TOut> dst = instance.YIsPositive((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        ClassicAssert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsPositiveInfinityTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector256s.GetSupportedMethodList<Func<Vector256<T>, Vector256<TOut>>>("YIsPositiveInfinity_Basic", "YIsPositiveInfinity_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector256<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector256<TOut> expected = Vector256s.YIsPositiveInfinity((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits256 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector256<TOut> dst = instance.YIsPositiveInfinity((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        ClassicAssert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsSubnormalTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector256s.GetSupportedMethodList<Func<Vector256<T>, Vector256<TOut>>>("YIsSubnormal_Basic", "YIsSubnormal_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector256<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector256<TOut> expected = Vector256s.YIsSubnormal((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits256 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector256<TOut> dst = instance.YIsSubnormal((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        ClassicAssert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsZeroTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector256s.GetSupportedMethodList<Func<Vector256<T>, Vector256<TOut>>>("YIsZero_Basic", "YIsZero_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector256<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector256<TOut> expected = Vector256s.YIsZero((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits256 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector256<TOut> dst = instance.YIsZero((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        ClassicAssert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
        ...
        public void YIsZeroOrSubnormalTest<T, TOut>(T src, TOut srcOut) where T : struct where TOut : struct {
            ...
            var funcList = Vector256s.GetSupportedMethodList<Func<Vector256<T>, Vector256<TOut>>>("YIsZeroOrSubnormal_Basic", "YIsZeroOrSubnormal_Bit");
            foreach (var func in funcList) {
                writer.WriteLine("{0}: OK", ReflectionUtil.GetShortNameWithType(func.Method));
            }
            writer.WriteLine();
            ...
            foreach (Vector256<T> value in samples) {
                writer.WriteLine(VectorTextUtil.Format("Sample:\t{0}", value));
                Vector256<TOut> expected = Vector256s.YIsZeroOrSubnormal((dynamic)value);
                writer.WriteLine(VectorTextUtil.Format("Expected:\t{0}", expected));
                foreach (IWVectorTraits256 instance in instances) {
                    if (!instance.GetIsSupported(true)) continue;
                    Vector256<TOut> dst = instance.YIsZeroOrSubnormal((dynamic)value);
                    if (allowLog || (showNotEquals && !expected.Equals(dst))) {
                        writer.WriteLine(VectorTextUtil.Format("{0}:\t{1}", instance.GetType().Name, dst));
                    } else {
                        ClassicAssert.AreEqual(expected, dst, $"{instance.GetType().Name}, value={value}");
