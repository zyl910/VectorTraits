...
namespace Zyl.VectorTraits.Benchmarks.AVector.YC {
    ...
    /// <summary>
    /// YClamp benchmark - Single.
    /// </summary>
#if NETCOREAPP3_0_OR_GREATER && DRY_JOB
    [DryJob]
#endif // NETCOREAPP3_0_OR_GREATER && DRY_JOB
    public partial class YClampBenchmark_Single : AbstractSharedBenchmark_Single {

        // -- var --
        private const TMy valueMin = 0.2f;
        private const TMy valueMax = 0.8f;
        ...
        /// <summary>
        /// Sum Clamp - Vector128 - Wasm - NoPseudo.
        /// </summary>
        /// <param name="src">Source array.</param>
        /// <param name="srcCount">Source count</param>
        /// <param name="shiftAmount">Shift amount.</param>
        /// <returns>Returns the sum.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe TMy StaticSumClampVector128_Wasm_NoPseudo(TMy[] src, int srcCount, TMy amin, TMy amax) {
            TMy rt = 0; // Result.
            int VectorWidth = Vector128<TMy>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
            ...
            fixed (TMy* p0 = &src[0]) {
                ...
                // Vector processs.
                for (i = 0; i < cntBlock; ++i) {
                    Vector128<TMy> vtemp = WVectorTraits128PackedSimd.Statics.YClamp(*(Vector128<TMy>*)p, vectorMin, vectorMax);
                    vrt = WVectorTraits128PackedSimd.Statics.Add(vrt, vtemp); // Add.
                    p += nBlockWidth;
                }
                // Remainder processs.
                for (i = 0; i < cntRem; ++i) {
        ...
        [Benchmark]
        public void SumClampVector128_Wasm_NoPseudo() {
            WVectorTraits128PackedSimd.Statics.ThrowForUnsupported(true);
            if (BenchmarkUtil.IsLastRun) {
                Volatile.Write(ref dstTMy, 0);
                //Debugger.Break();
            }
            dstTMy = StaticSumClampVector128_Wasm_NoPseudo(srcArray, srcArray.Length, valueMin, valueMax);
            CheckResult("SumClampVector128_Wasm_NoPseudo");
        }

        /// <summary>
        /// Sum Clamp - Vector128 - Wasm - Pseudo.
        /// </summary>
        /// <param name="src">Source array.</param>
        /// <param name="srcCount">Source count</param>
        /// <param name="shiftAmount">Shift amount.</param>
        /// <returns>Returns the sum.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe TMy StaticSumClampVector128_Wasm_Pseudo(TMy[] src, int srcCount, TMy amin, TMy amax) {
            TMy rt = 0; // Result.
            int VectorWidth = Vector128<TMy>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
        ...
        [Benchmark]
        public void SumClampVector128_Wasm_Pseudo() {
            WVectorTraits128PackedSimd.Statics.ThrowForUnsupported(true);
            if (BenchmarkUtil.IsLastRun) {
                Volatile.Write(ref dstTMy, 0);
                //Debugger.Break();
            }
            dstTMy = StaticSumClampVector128_Wasm_Pseudo(srcArray, srcArray.Length, valueMin, valueMax);
            CheckResult("SumClampVector128_Wasm_Pseudo");
        }
