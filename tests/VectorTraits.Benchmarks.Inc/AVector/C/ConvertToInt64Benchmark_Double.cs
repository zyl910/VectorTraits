...
namespace Zyl.VectorTraits.Benchmarks.AVector.C {
    ...
    public partial class ConvertToInt64Benchmark_Double : AbstractSharedBenchmark {
        ...
        /// <summary>
        /// Sum ConvertToInt64 - 256 - Avx2 - Veloctor.
        /// </summary>
        /// <param name="src">Source array.</param>
        /// <param name="srcCount">Source count</param>
        /// <returns>Returns the sum.</returns>
        public static TMyOut StaticSum256Avx2_Veloctor(TMy[] src, int srcCount) {
            TMyOut rt = 0; // Result.
            int Vector256Width = Vector256<TMy>.Count; // Block width.
            int nBlockWidth = Vector256Width; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
            ...
            // a) Vector processs.
            for (i = 0; i < cntBlock; ++i) {
                vtemp = WVectorTraits256Avx2.Statics.ConvertToInt64_Veloctor(p0);
                vrt = Vector256s.Add(vrt, vtemp);
                p0 = ref Unsafe.Add(ref p0, 1);
            }
            // b) Remainder processs.
        ...
        [Benchmark]
        public void Sum256Avx2_Veloctor() {
            WVectorTraits256Avx2.Statics.ThrowForUnsupported(true);
            if (BenchmarkUtil.IsLastRun) {
                Volatile.Write(ref dstTMy, 0);
                //Debugger.Break();
            }
            dstTMy = StaticSum256Avx2_Veloctor(srcArray, srcArray.Length);
            CheckResult("Sum256Avx2_Veloctor");
        }
