...
namespace Zyl.VectorTraits.Benchmarks.AVector.S {
    ...
    internal partial class ShiftRightLogicalBenchmark_UInt16 : AbstractSharedBenchmark_UInt16 {
        ...
        private static unsafe TMy StaticSumSRLFast_Widen(TMy[] src, int srcCount, int shiftAmount) {
            ...
            fixed (TMy* p0 = &src[0]) {
                ...
                // Vector processs.
                for (i = 0; i < cntBlock; ++i) {
                    Vector<TMy> vtemp = VectorTraitsBase.Statics.ShiftRightLogicalFast_Widen(*(Vector<TMy>*)p, shiftAmount);
                    vrt += vtemp; // Add.
                    p += nBlockWidth;
                }
                // Remainder processs.
        ...
        private static unsafe TMy StaticSumSRLFast_Widen128(TMy[] src, int srcCount, int shiftAmount) {
            ...
            fixed (TMy* p0 = &src[0]) {
                ...
                // Vector processs.
                for (i = 0; i < cntBlock; ++i) {
                    Vector<TMy> vtemp = VectorTraits128Base.Statics.ShiftRightLogicalFast_Widen(*(Vector<TMy>*)p, shiftAmount);
                    vrt += vtemp; // Add.
                    p += nBlockWidth;
                }
                // Remainder processs.
        ...
        private static unsafe TMy StaticSumSRLFast_Widen256(TMy[] src, int srcCount, int shiftAmount) {
            ...
            fixed (TMy* p0 = &src[0]) {
                ...
                // Vector processs.
                for (i = 0; i < cntBlock; ++i) {
                    Vector<TMy> vtemp = VectorTraits256Base.Statics.ShiftRightLogicalFast_Widen(*(Vector<TMy>*)p, shiftAmount);
                    vrt += vtemp; // Add.
                    p += nBlockWidth;
                }
                // Remainder processs.
