...
namespace Zyl.VectorTraits.Benchmarks.AVector.S {
    ...
    public partial class ShiftRightLogicalBenchmark_UInt64 : AbstractSharedBenchmark_UInt64 {
        ...
        private static unsafe TMy StaticSumSRLFastRawAdvSimdImm(TMy[] src, int srcCount, int shiftAmount) {
            ...
            fixed (TMy* p0 = &src[0]) {
                ...
                if (0 != shiftAmount) {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<TMy> vtemp = VectorTraits128AdvSimd.Statics.ShiftRightLogical_Fast_HwImm(*(Vector<TMy>*)p, shiftAmount);
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                } else {
        ...
        private static unsafe TMy StaticSumSRLFastRawAdvSimdVar(TMy[] src, int srcCount, int shiftAmount) {
            ...
            fixed (TMy* p0 = &src[0]) {
                ...
                if (0 != shiftAmount) {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<TMy> vtemp = VectorTraits128AdvSimd.Statics.ShiftRightLogical_Fast_HwVar(*(Vector<TMy>*)p, shiftAmount);
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                } else {
