...
namespace Zyl.VectorTraits.Benchmarks.AVector.S {
    ...
    /// <summary>
    /// ShiftRightArithmetic benchmark - Int64.
    /// </summary>
    ...
    public partial class ShiftRightArithmeticConstBenchmark_Int64 : AbstractSharedBenchmark_Int64 {
        ...
        /// <summary>
        /// Sum ShiftRightArithmetic - Base - Create.
        /// </summary>
        /// <param name="src">Source array.</param>
        /// <param name="srcCount">Source count</param>
        /// <param name="shiftAmount">Shift amount.</param>
        /// <returns>Returns the sum.</returns>
        private static TMy StaticSumSRA_Base_Create(TMy[] src, int srcCount, int shiftAmount) {
            TMy rt = 0; // Result.
            const int GroupSize = 1;
            int VectorWidth = Vector<TMy>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            ...
            // Vector processs.
            for (i = 0; i < cntBlock; ++i) {
                Vector<TMy> vtemp = VectorTraitsBase.Statics.ShiftRightArithmetic_Fast_Negative_Create(p0, shiftAmount);
                vrt += vtemp; // Add.
                p0 = ref Unsafe.Add(ref p0, GroupSize);
            }
            // Remainder processs.
        ...
        [Benchmark]
        public void SumSRA_Base_Create() {
            VectorTraitsBase.Statics.ThrowForUnsupported(true);
            if (BenchmarkUtil.IsLastRun) {
                Volatile.Write(ref dstTMy, 0);
                //Debugger.Break();
            }
            dstTMy = StaticSumSRA_Base_Create(srcArray, srcArray.Length, shiftAmount);
            CheckResult("SumSRA_Base_Create");
        }

        /// <summary>
        /// Sum ShiftRightArithmetic - Base - LoadArray.
        /// </summary>
        /// <param name="src">Source array.</param>
        /// <param name="srcCount">Source count</param>
        /// <param name="shiftAmount">Shift amount.</param>
        /// <returns>Returns the sum.</returns>
        private static TMy StaticSumSRA_Base_LoadArray(TMy[] src, int srcCount, int shiftAmount) {
            TMy rt = 0; // Result.
            const int GroupSize = 1;
            int VectorWidth = Vector<TMy>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            ...
            // Vector processs.
            for (i = 0; i < cntBlock; ++i) {
                Vector<TMy> vtemp = VectorTraitsBase.Statics.ShiftRightArithmetic_Fast_Negative_LoadArray(p0, shiftAmount);
                vrt += vtemp; // Add.
                p0 = ref Unsafe.Add(ref p0, GroupSize);
            }
            // Remainder processs.
        ...
        [Benchmark]
        public void SumSRA_Base_LoadArray() {
            VectorTraitsBase.Statics.ThrowForUnsupported(true);
            if (BenchmarkUtil.IsLastRun) {
                Volatile.Write(ref dstTMy, 0);
                //Debugger.Break();
            }
            dstTMy = StaticSumSRA_Base_LoadArray(srcArray, srcArray.Length, shiftAmount);
            CheckResult("SumSRA_Base_LoadArray");
        }
        ...
        /// <summary>
        /// Sum shift right arithmetic - Bcl static.
        /// </summary>
        /// <param name="src">Source array.</param>
        /// <param name="srcCount">Source count</param>
        /// <param name="shiftAmount">Shift amount.</param>
        /// <returns>Returns the sum.</returns>
        private static TMy StaticSumSRA512Bcl(TMy[] src, int srcCount, int shiftAmount) {
            TMy rt = 0; // Result.
            const int GroupSize = 1;
            ...
            // Vector processs.
            for (i = 0; i < cntBlock; ++i) {
                Vector512<TMy> vtemp = Vector512.ShiftRightArithmetic(p0, shiftAmount);
        ...
        [Benchmark]
        public void SumSRA512Bcl() {
            if (BenchmarkUtil.IsLastRun) {
                Volatile.Write(ref dstTMy, 0);
                //Debugger.Break();
            }
            dstTMy = StaticSumSRA512Bcl(srcArray, srcArray.Length, shiftAmount);
            CheckResult("SumSRA512Bcl");
        }

        /// <summary>
        /// Sum shift right arithmetic - Traits static.
        /// </summary>
        /// <param name="src">Source array.</param>
        /// <param name="srcCount">Source count</param>
        /// <param name="shiftAmount">Shift amount.</param>
        /// <returns>Returns the sum.</returns>
        private static TMy StaticSumSRA512Traits(TMy[] src, int srcCount, int shiftAmount) {
            TMy rt = 0; // Result.
            const int GroupSize = 1;
            int VectorWidth = Vector512<TMy>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            ...
            // Vector processs.
            for (i = 0; i < cntBlock; ++i) {
                Vector512<TMy> vtemp = Vector512s.ShiftRightArithmetic(p0, shiftAmount);
        ...
        [Benchmark]
        public void SumSRA512Traits() {
            Vector512s.ThrowForUnsupported(true);
            if (BenchmarkUtil.IsLastRun) {
                Volatile.Write(ref dstTMy, 0);
                //Debugger.Break();
            }
            dstTMy = StaticSumSRA512Traits(srcArray, srcArray.Length, shiftAmount);
            CheckResult("SumSRA512Traits");
        }
