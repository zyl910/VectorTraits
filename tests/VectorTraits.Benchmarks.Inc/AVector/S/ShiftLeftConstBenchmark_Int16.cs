...
namespace Zyl.VectorTraits.Benchmarks.AVector.S {
    ...
    public partial class ShiftLeftConstBenchmark_Int16 : AbstractSharedBenchmark_Int16 {
        ...
        /// <param name="shiftAmount">Shift amount.</param>
        /// <returns>Returns the sum.</returns>
        private static TMy StaticSumSLL_Base(TMy[] src, int srcCount, int shiftAmount) {
            TMy rt = 0; // Result.
            const int GroupSize = 1;
            int VectorWidth = Vector<TMy>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
        ...
        [Benchmark]
        public void SumSLL_Base() {
            VectorTraitsBase.Statics.ThrowForUnsupported(true);
            dstTMy = StaticSumSLL_Base(srcArray, srcArray.Length, shiftAmount);
        ...
        private static TMy StaticSumSLL_Base_Core_Element(TMy[] src, int srcCount, int shiftAmount) {
            ...
            int i;
            // Body.
            var args0 = VectorTraitsBase.Statics.ShiftLeft_Args_Element(vrt, shiftAmount, out var args1);
            ref Vector<TMy> p0 = ref Unsafe.As<TMy, Vector<TMy>>(ref src[0]);
            // Vector processs.
            for (i = 0; i < cntBlock; ++i) {
                Vector<TMy> vtemp = VectorTraitsBase.Statics.ShiftLeft_Core_Element(p0, args0, args1);
                vrt += vtemp; // Add.
                p0 = ref Unsafe.Add(ref p0, GroupSize);
            }
            // Remainder processs.
        ...
        /// <summary>
        /// Sum shift left logical - Base - Core - Mov.
        /// </summary>
        /// <param name="src">Source array.</param>
        /// <param name="srcCount">Source count</param>
        /// <param name="shiftAmount">Shift amount.</param>
        /// <returns>Returns the sum.</returns>
        private static TMy StaticSumSLL_Base_Core_Mov(TMy[] src, int srcCount, int shiftAmount) {
            TMy rt = 0; // Result.
            const int GroupSize = 1;
            int VectorWidth = Vector<TMy>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            ...
            int i;
            // Body.
            var args0 = VectorTraitsBase.Statics.ShiftLeft_Args_Mov(vrt, shiftAmount, out var args1);
            ref Vector<TMy> p0 = ref Unsafe.As<TMy, Vector<TMy>>(ref src[0]);
            // Vector processs.
            for (i = 0; i < cntBlock; ++i) {
                Vector<TMy> vtemp = VectorTraitsBase.Statics.ShiftLeft_Core_Mov(p0, args0, args1);
                vrt += vtemp; // Add.
                p0 = ref Unsafe.Add(ref p0, GroupSize);
            }
            // Remainder processs.
        ...
        [Benchmark]
        public void SumSLL_Base_Core_Mov() {
            VectorTraitsBase.Statics.ThrowForUnsupported(true);
            if (BenchmarkUtil.IsLastRun) {
                Volatile.Write(ref dstTMy, 0);
                //Debugger.Break();
            }
            dstTMy = StaticSumSLL_Base_Core_Mov(srcArray, srcArray.Length, shiftAmount);
            CheckResult("SumSLL_Base_Core_Mov");
        }
        ...
        private static TMy StaticSumSLLTraits_Core(TMy[] src, int srcCount, int shiftAmount) {
            ...
            // Vector processs.
            for (i = 0; i < cntBlock; ++i) {
                Vector<TMy> vtemp = Vectors.ShiftLeft_Core(p0, args0, args1);
                vrt += vtemp; // Add.
                p0 = ref Unsafe.Add(ref p0, GroupSize);
            }
            // Remainder processs.
