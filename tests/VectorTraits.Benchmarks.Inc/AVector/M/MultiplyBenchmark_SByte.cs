...
namespace Zyl.VectorTraits.Benchmarks.AVector.M {
    ...
    public partial class MultiplyBenchmark_SByte : AbstractSharedBenchmark_SByte {
        ...
        /// <param name="srcCount">Source count</param>
        /// <returns>Returns the sum.</returns>
        public static TMy StaticSumBcl_Widen(TMy[] src, int srcCount) {
            TMy rt = 0; // Result.
            const int GroupSize = 2;
            int VectorWidth = Vector<TMy>.Count; // Block width.
            int nBlockWidth = VectorWidth * GroupSize; // Block width.
        ...
        [Benchmark]
        public void SumBcl_Widen() {
            dstTMy = StaticSumBcl_Widen(srcArray, srcArray.Length);
            CheckResult("SumBcl_Widen");
        }
