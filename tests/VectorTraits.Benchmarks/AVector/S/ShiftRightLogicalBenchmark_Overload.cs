    ...
    partial class ShiftRightLogicalBenchmark_Byte {
        ...
        private static unsafe Byte StaticSumSRLFastTraitsOverload(VectorTraitsBase vectorTraits, Byte[] src, int srcCount, int shiftAmount) {
            ...
            fixed (Byte* p0 = &src[0]) {
                ...
                if (0 != shiftAmount) {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<Byte> vtemp = vectorTraits.ShiftRightLogical_Fast(*(Vector<Byte>*)p, shiftAmount);
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                } else {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<Byte> vtemp = *(Vector<Byte>*)p;
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                }
        ...
        // StaticSumSRLTraitsOverload - VectorTraits128AdvSimdB64
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe Byte StaticSumSRLTraitsOverload(VectorTraits128AdvSimdB64 vectorTraits, Byte[] src, int srcCount, int shiftAmount) {
            Byte rt = 0; // Result.
            int VectorWidth = Vector<Byte>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
        ...
        // StaticSumSRLFastTraitsOverload - VectorTraits128AdvSimdB64
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe Byte StaticSumSRLFastTraitsOverload(VectorTraits128AdvSimdB64 vectorTraits, Byte[] src, int srcCount, int shiftAmount) {
            Byte rt = 0; // Result.
            int VectorWidth = Vector<Byte>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
    ...
    partial class ShiftRightLogicalBenchmark_UInt16 {
        ...
        private static unsafe UInt16 StaticSumSRLFastTraitsOverload(VectorTraitsBase vectorTraits, UInt16[] src, int srcCount, int shiftAmount) {
            ...
            fixed (UInt16* p0 = &src[0]) {
                ...
                if (0 != shiftAmount) {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<UInt16> vtemp = vectorTraits.ShiftRightLogical_Fast(*(Vector<UInt16>*)p, shiftAmount);
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                } else {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<UInt16> vtemp = *(Vector<UInt16>*)p;
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                }
        ...
        // StaticSumSRLTraitsOverload - VectorTraits128AdvSimdB64
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt16 StaticSumSRLTraitsOverload(VectorTraits128AdvSimdB64 vectorTraits, UInt16[] src, int srcCount, int shiftAmount) {
            UInt16 rt = 0; // Result.
            int VectorWidth = Vector<UInt16>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
        ...
        // StaticSumSRLFastTraitsOverload - VectorTraits128AdvSimdB64
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt16 StaticSumSRLFastTraitsOverload(VectorTraits128AdvSimdB64 vectorTraits, UInt16[] src, int srcCount, int shiftAmount) {
            UInt16 rt = 0; // Result.
            int VectorWidth = Vector<UInt16>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
    ...
    partial class ShiftRightLogicalBenchmark_UInt32 {
        ...
        private static unsafe UInt32 StaticSumSRLFastTraitsOverload(VectorTraitsBase vectorTraits, UInt32[] src, int srcCount, int shiftAmount) {
            ...
            fixed (UInt32* p0 = &src[0]) {
                ...
                if (0 != shiftAmount) {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<UInt32> vtemp = vectorTraits.ShiftRightLogical_Fast(*(Vector<UInt32>*)p, shiftAmount);
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                } else {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<UInt32> vtemp = *(Vector<UInt32>*)p;
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                }
        ...
        // StaticSumSRLTraitsOverload - VectorTraits128AdvSimdB64
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt32 StaticSumSRLTraitsOverload(VectorTraits128AdvSimdB64 vectorTraits, UInt32[] src, int srcCount, int shiftAmount) {
            UInt32 rt = 0; // Result.
            int VectorWidth = Vector<UInt32>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
        ...
        // StaticSumSRLFastTraitsOverload - VectorTraits128AdvSimdB64
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt32 StaticSumSRLFastTraitsOverload(VectorTraits128AdvSimdB64 vectorTraits, UInt32[] src, int srcCount, int shiftAmount) {
            UInt32 rt = 0; // Result.
            int VectorWidth = Vector<UInt32>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
    ...
    partial class ShiftRightLogicalBenchmark_UInt64 {
        ...
        private static unsafe UInt64 StaticSumSRLFastTraitsOverload(VectorTraitsBase vectorTraits, UInt64[] src, int srcCount, int shiftAmount) {
            ...
            fixed (UInt64* p0 = &src[0]) {
                ...
                if (0 != shiftAmount) {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<UInt64> vtemp = vectorTraits.ShiftRightLogical_Fast(*(Vector<UInt64>*)p, shiftAmount);
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                } else {
                    for (i = 0; i < cntBlock; ++i) {
                        Vector<UInt64> vtemp = *(Vector<UInt64>*)p;
                        vrt += vtemp; // Add.
                        p += nBlockWidth;
                    }
                }
        ...
        // StaticSumSRLTraitsOverload - VectorTraits128AdvSimdB64
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt64 StaticSumSRLTraitsOverload(VectorTraits128AdvSimdB64 vectorTraits, UInt64[] src, int srcCount, int shiftAmount) {
            UInt64 rt = 0; // Result.
            int VectorWidth = Vector<UInt64>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
        ...
        // StaticSumSRLFastTraitsOverload - VectorTraits128AdvSimdB64
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe UInt64 StaticSumSRLFastTraitsOverload(VectorTraits128AdvSimdB64 vectorTraits, UInt64[] src, int srcCount, int shiftAmount) {
            UInt64 rt = 0; // Result.
            int VectorWidth = Vector<UInt64>.Count; // Block width.
            int nBlockWidth = VectorWidth; // Block width.
            int cntBlock = srcCount / nBlockWidth; // Block count.
